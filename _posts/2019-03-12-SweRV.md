...
under a liberal Apache 2.0 open source license, with instruction set simulator (ISS) and testbenches included as well!
...
* During their demo of the SweRV on an Artix 7 FPGA, they were able to reach a clock speed of around 40MHz, with 
  ...
  combinatorial gates. A VexRiscV, which was designed for FPGA, would run circles around a SweRV in terms of 
  clock speed.
* The RTL is written in SystemVerilog, using some constructs like port assignemnt wildcards that are not (yet)
  supported by Yosys. This eliminates open source flows as a potential candidate.
* Finally, the SweRV core is big: it'd probably fill up 50% of a relatively large FPGA, though that needs to be
  tested in practice.
