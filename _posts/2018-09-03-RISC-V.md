...
see where the VecRiscv would end up in a similar configuration as the MR1 and the picorv32.

I chose a configuration with bypass logic where possible, so it has the best IPC of all 3.

Result:

* 106 MHz (VexRiscv) vs 82 MHz (mr1) vs 99MHz (picorv32)
* 800 LEs vs 1259 vs 1582

In other words: it's smaller, it synthesizes faster, and it has better IPC.
