...
It adds an pass-through Fetch block and, more important, adds
...
* Check in the log file for the offending assert.

    In the case of the `complete` test, this is very easy. But firing up the waveform viewer without knowing exactly what went wrong is
    a waste of time.

* Run the `disasm.py` script on the VCD file to create a list of executed instruction

    In order to understand the expected behavior, you need to know which instructions were running and which one failed.

    The first two weeks, I did that by decoding the instruction words manually, with the RISC-V specification in hand. Turns
...
* Compare the `rvfi_*` signals of the offending instruction with the expected value.
    ...
    For most other tests, you also need to compare rvfi and spec signals.

* Once you see the difference, you can start digging in waveforms and RTL and fix the bug.

* Rerun the test.

    This is where you realize one of the negative points of formal proofs: it's sufficient to change on little thing in your design to
    end up with a completely different failing case, even if that little thing didn't actually fix your bug.

    When running simulations, it's often a matter of progressively peeling the onion: first it runs for a 100 cycles, then 110 cycles
    after fixing something simple, then it runs for 1000 cycles, then 10000, until it the test is fully passing.
    ...
    With formal, this is not true (unless you overconstrain the input with `assume` statements.) After each fix, you need to find your
    bearings again and figure out what's going wrong this time.

    This would be pretty bad if it weren't for the fact that formal is much better at zeroing in on a bug very quickly: it can't simulate
    ...
    it much easier to figure what's going wrong.
...
That work included setting up a SpinalHDL project, baby steps in getting started writing my first RTL in SpinalHDL, figuring out how to
set up and run riscv-formal on my own design, debugging the decoder.

The actual debug from first formal run to `complete` test fully passing only took about about an hour or so.

Amazing part is that it has been stress tested under all possible conditions. No corner cases where left unturned.
