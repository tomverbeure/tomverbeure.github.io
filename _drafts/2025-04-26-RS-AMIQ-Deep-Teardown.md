...
When you look at the analog signal path from DAC to output, you can see the DAC, the anti-aliasing
filters, and output section that does amplification and attenuation and finally the output connector
board.

It is common for signal generators to have a fixed amplification section and then some selectable
fixed value attenuation stages: it is difficult to design a amplifier with a variable gain with
low distortion. If you need a signal amplitude that can't be achieved with one of the fixed attenuation
settings, one solution is to multiply the signal that enters the DAC to reduce the DAC output amplitude.
 
This is not the case for the AMIQ: while it can use a signal path that only uses fixed amplification and attenuation
stages, it also offers the intriguing option to send the analog signal through a variable multipler stage:

![Variable gain amplifier](/assets/amiq/variable_gain_unit.png)

If we zoom down from the system diagram to the block diagram, we can see how multiplier/variable
...
W back into Z either. The functionality reduces to: `W = FILTER_OUT * AMPL_VAR + OFFSET`.
...
level of signals that are way higher than the sample rate of the ADC.
