---
layout: post
title: Controlling a HP 1670G Logic Analyzer over Ethernet
...
Earlier this year, I bought an HP 1670G logic analyzer at the 
[Silicon Valley Electronics Flea Market](https://www.electronicsfleamarket.com/). 
With a production date sometime early 2000, it's new enough to have an Agilent sticker, and
it's in pristine condition. Unlike most other HP logic analyzers, this one doesn't have
expansion cards: what you see is what you get. But 136 channels that can be sampled
at 500MHz is plenty, and my machine has the oscilloscope option: 2 channels, 500MHz
bandwidth and 2GSa/s are very nice specs indeed.

The 1670G is one of the last HP models that runs on a custom operating system, later models
switched to HP/UX, but it has some advanced features, such as being to operate the UI 
through an X-server. And that's great because operating the UI with the keys and rotary knobs
on the front panel is not a pleasant experience. You can connect a keyboard and mouse, but 
who has extra space for that on their bench or equipment cart?

It used to be easy to have any kind of X client connect to a remove X server: back in the
early 1990s, on the HP Apollo workstations in college, it was a simple matter of setting
the DISPLAY environment variable, but things are bit more complicated now for obvious
security reasons.

It took me a couple of hours to get things up and running on my Ubuntu 20.04 machine. In
this blog post, I got through the steps that were required to make things work so that I
...
# Reserving a fixed IP address for your logic analyzer

Like most other people, tens of my household's PCs, laptops, and IOT devices are
assigned a dynamic IP address with DHCP. The 1670G only supports static IP address.
...
dynamic IP addresses are, well, dynamic, they often stay the same for a long time. Just
choose an address that's different that all the other and hope for the best. It's not as if
you'll be using the machine like this 24/7...*
...
After loggin in, assign a fixed IP address for the logic analyzer:
...
`socat` is "a multipurpose relay tool for Linux". It's an abbreviation of "SOcket CAT". It
...
The logic analyzer will send X server request to TCP/IP port 6111: the display number. 

We need to tell the firewall to not block these inconming request.
...
The logic analyzer request to port 6111 are rerouted to the active X display with `socat`:

```sh
socat TCP-LISTEN:6111,fork,bind=192.168.1.51 UNIX-CONNECT:/tmp/.X11-unix/X1
```

There 3 keyvalues here are:
...
Go to the X-Window Settings configuration screen: System -> External IO -> X Settings
...
HP provides special font files for your X server. They can be found on the HD
...
Copy these files from the logic analyzer to your local machine with FTP (which didn't
work for me), or just download them by clicking:
[`LG165.BDF`](/assets/hp1670g/LG165.BDF), [`SG165.BDF`](/assets/hp1670g/LG165.BDF), 
[`README`](/assets/hp1670g/README). Store the file on your Linux machine's file system.
...
You will need to do the last 2 command after each reboot...

The README has instructions on how to make these changes permanent, but that didn't 
work on my machine.
