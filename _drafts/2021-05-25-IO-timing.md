...
1. The link (the USB controller) issues a RegRead command on the data bus. It does this by setting
...
1. The PHY asserts `dir` to turn around the direction of the data bus and take control of it. However,
   it doesn't drive the data bus yet.
...
* The PHY creates the clock and sends it to the link. This is called ULPI *Output Clock* mode.
  ...
  Arrow DECA board is configured in output clock mode.
...
(not Xilinx ISE!), timing constraints are supplied an `.sdc` file, short for
Synospys Design Constraint file. A SDC file is really just TCL file (the world's most
...
* hold time
    ...
    The hold time is the minium timine during which a signal must be stable
...
1. Check if the path between 2 flip-flops must be analysed.

    The timing constraints will often mark paths as false paths, for example because
    the designer has indicated that paths between 2 asynchronous clock domains should
    not be checked.
...
1. Find the shortest possible positive delay between any possible launch edge and any possible
   ...
   timing analyzer choose the smallest possible delay between the two edges:
    ...
    at their source, **without taking into account the clock delay from the source of the
...
1. Apply an optional multi-cycle adjustment to the setup time.
    ...
    In the diagram below, the latch clock is shifted 60[^3] degrees ahead of the launch clock. If 
    the logic was designed with the intention that the data is latched 7ns instead of 1ns after
    being launched, a 1-cycle multi-cycle adjustment must be applied:

    [^3]: 1ns / 6ns * 360 = 60

    ![setup multi-cycle path](/assets/io_timings/io_timings-setup_multi_cycle_path.svg)

    One reason to have such a phase shifted clock is to relax setup timing. In the example
    above, the delay from the launching to the latching FF can be 1ns higher before
    violating the setup time. This technique is commonly used when dealing with 
    external IO interfaces.


1. Calculate the launch to latch delay: delay from the starting point of the launch clock to the latching 
   point of the latching flip-flop

    ![setup launch to latch delay](/assets/io_timings/io_timings-setup_launch_to_latch_delay.svg)

1. Calculate the latch delay: delay fro the starting point of the latch clock to the latching
   flip-flop
