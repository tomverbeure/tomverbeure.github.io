...
* Install another RISC-V toolchain
    ...
    The `riscv` design compiles into the default bitstream that's flashed when you initially receive the board. 
    It makes the LED cycle through all the colors of the rainbow. This design is quite complex. It even includes 
    a RISC-V soft CPU. The build flow needs a RISC-V GCC compiler to generate the firmware for this example design.

    In theory, you should be able to reuse the RISC-V compiler that's used to compile the ESP32C3 firmware, but
