...
* the HP part number. Many components, even generic ones, were custom marked for HP with their
  unique number. And sometimes, the generic name is dropped altogether. Here's an example on the controller
 ...
 [HP Bench Brief](/assets/hp16500a/datasheets/HP-Bench-Briefs-1990-07-12.pdf)! Bench briefs
  ...
  about the inner workings of HP equipment, trouble shooting tips, as well as lists with HP 
  product numbers and their generic name. Like this:
...
I expected everything to be very conventional (and it is), but I still wanted to get a better idea 
about what exactly made the logic analyzer tick. So I started annotating a few components... 
...
The only unknown is IC 1FI1-0002. Based on its location and, by elimination, on the block diagram,
...
* 32K x 16 bits EPROM 
    ...
    *for a whopping 15 to 20 minutes*.
    ...
    I bought a TL866-II EPROM reader ($61 on AliExpress) to dump the contents of the EPROMs for
    analysis and reverse engineering in Ghidra. It worked great, but soon after doing that somebody
...
* 256 KByte of dual-ported graphics DRAM
    ...
    Built out of 8 banks of 64K x 4 bit NEC D41264C-12 ICs. ([Datasheet](/assets/hp16500a/datasheets/NECED001-12A-1.pdf))

    Dual-ported DRAMs make it much easier to design a frame buffer without needing to worry about interference between
    the display scanout logic and the CPU.
    ...
    Dual-ported graphics DRAM was a thing until sometime in the nineties, but has since completely disappeared:
    Not only were they signficantly more expensive, but also because the need for them has gone. With increasing
    clock speeds, the bandwidth needed to read out pixels from the frame buffer became an ever smaller part, and 
    all the logic to arbitrate requested between multiple agents was pulled into controller ICs.

* 8 4-bit parallel shift registers

    [74F195N 4-bit parallel-access shift register](/assets/hp16500a/datasheets/74F195A.pdf)) capture 
...
* 3x 4-bit video DAC with 16 color palette
    ...
    The NE5150F ([datasheet](/assets/hp16500a/datasheets/NE5150.pdf)) takes in a 4 address bits to look up 
...
* 256 x 4 bits SRAM
    ...
    This RAM, a CY7C122-25PC ([datasheet](/assets/hp16500a/datasheets/cy7c122.pdf)), is a bit of a mystery:
...
* CRT controller
    ...
    An MC68A45 ([datasheet](/assets/hp16500a/datasheets/mc68A45_datasheet.pdf)) is the final piece
    of the display logic. While the previous chips were all part of the graphics data pipeline,
    this IC contains the main display control logic: video memory addressing and video timing generatio such as vsync
    and hsync.

    The 6845 can also generate addresses for a character ROM (for those cases when you don't use a bitmapped
    video memory), but I don't think that part is being used on this board.

# Communication Devices

* GPIB Controller

    A TMS9914A ([datasheet](/assets/hp16500a/datasheets/TMS9914A_General_Purpose_Interface_Bus_Controller_Data_Manual_Dec82.pdf))
    is used as GPIB Controller.

    ![TMS9914A block diagram](/assets/hp16500a/blog_disassembly/38_tms9914a_block_diagram.jpg)

    It implements the GPIB protocol statemachine and thus relieves the CPU from having to takes care of the 
...
# HP Human Interface Link

* HP-HIL Host Controller
    ...
    On the 16500A, the embedded touch screen counts as a one HIL device. Further devices such as the
    keyboard can be plugged in the jack that's on the front panel.
