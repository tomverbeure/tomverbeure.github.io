...
to make that happen is establish communication between my PC and the 1992 using GPIB. It sounds like 
something that's relatively easy to do, but some well-placed booby-traps turned it into something that took
...
The problem was that this didn't work *at all*. When I'd send the internal self-test `CK` command,
...
Opening up the 1992 can be done by removing just 2 screws, after which you can slide out the bottom part 
...
commands keep on reading until it sees `EOI`, even if you specify a size of 21 with `read_raw()`! 
...
On my system, all the example code worked fine... until it didn't. I don't know what happened, but
...
My 1992 was not able to recover from it. Sending the instrument preset `IP` command,
power cycling the unit, rebooting the PC, unplugging the GPIB dongle, none of that
worked.
...
It's a bit unsettling when something that used work perfectly fine suddenly doesn't,
