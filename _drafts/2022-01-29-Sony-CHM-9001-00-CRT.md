...
Almost all monitors old and new have square pixels. That's not the case here: the stated active 
area of 160x120mm has an aspect ratio of 1.33, but active resolution is specified at 576x368 or a 
ratio of 1.57. This would be really annoying on an LCD display, but it's far less so on a CRT where
individual pixels blend into each other anyway. If you want to drive the CRT with square pixels, 
you could lower the pixel clock from 20 MHz to 16.9MHz (multiply by 1.33/1.57) and reduce the
horizontal resolution from 576 to 485, and the analog-only circuitry of the CRT wouldn't have 
a clue that this happened because all the timings would remain the same. The image quality
might be a little bit lower because the CRT RGB raster won't be the native resolution.
