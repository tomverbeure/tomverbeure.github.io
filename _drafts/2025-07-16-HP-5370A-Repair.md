...
thus shutting them off. *In the schematic above, the +15V power transistor is on the right, the
current control transistor on the left, and current sense resistor R4 is right next to the
+15V label.*
...
One thing that had bothered me during PCB swapping was why the front-panel display had remained off 
when all CPU boards were plugged in. Unlike later HP test equipment like the HP 5334A universal counter, 
the CPU clock of the 5370A is derived from the 10 MHz clock that comes out of this reference frequency 
buffer PCB[^div8], so if this board is broken, nothing works.
...
interesting: I assumed that it would work by creating some second harmonic and filter
out the base frequency, and that's kind of it works.

![Injection locked oscillator with 3 10MHz LC tanks](/assets/hp5370a/ref_freq_buffer_injection_locked_oscillator.jpg)

There are 3 LC tanks with an inductions of 1 uH and a capacitance of 250pF, good for a natural
resonance frequency of $$f = \frac{1}{2 \pi \sqrt{ L C }}$$ = 10.066 MHz. The incoming 5 MHz or 10 MHz 
signal periodically inserts a bit of energy into the LC tank and nudges it to be in sync with it. 
This circuit deserves a blog post on its own.
...
* [EEVblog forum thread about frequency multipliers](https://www.eevblog.com/forum/beginners/making-a-frequency-doubler-circuit-want-another-set-of-eyes-on-it/)
