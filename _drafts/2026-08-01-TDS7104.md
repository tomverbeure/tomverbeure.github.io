...
The CD-224E has 50-pin slimline IDE connector, but there's a small adapter PCB to make it
work with a standard PATA/IDE cable.
...
to make sure the issue was with the drive and not the CDROM disc, and, indeed, that
didn't work either. With the new CD-224E/dongle combo, my laptop could read the installation
...
There are many different version of the CD-224E, all with the same dimensions and interface,
but clearly they don't behave the same. The version of the broken one is version A93 (2000),
the new one is CD0 (2005). You can find A93 drives on eBay, but $69 is way too high from
something that I'd be using exactly once.
...
by using a virtual machine. The process is convoluted:
...
I once again spend many hours getting this to work, it never did.



# Burning the Windows 2000 Pro Installation Disk onto a USB Stick

Most of the time, you can use a utility like Balena Etcher to burn a CDROM
ISO onto a USB stick, but that doesn't work for the Windows 2000 Pro installation
CDROM.

Instead, you need to use a tool like ... to prepare the USB stick.

# Booting from USB Stick with a Plot Linux Boot Manager

As mentioned ealier, the RadiSys motherboard doesn't natively support booting from
USB stick, but there is hack to make it work after all, through the Plop Boot Manager.
...
The boot manager can be found here:

I tried hard to avoid going this route because my experience with floppy drives on
old test equipment has been absymal: none of them worked. Having no choice, I once
against tried to copy the boot manager image to my USB floppy drive and... that didn't
work either. All these years it was the USB floppy drive, freshly bought from Amazon,
was the culprit!

Since the scope still worked fine with the IBM HD, I use the its own floppy drive to
put the image onto the floppy disc.

After setting the BIOS to allow booting from floppy, the scope booted into the
Plop Boot Manager just fine and in turns it was able to boot the USB stick with
Windows 2000 ISO.

# Installing Windwos 2000 Pro
