...
There are two interesting components in the clock generation path:
...
commentary about the specification, but based on some comparisons of similar components on Digikey, such
...
The AD9850 is located in the feedback path of the PLL where it acts as
an output clock divider with a precision of 32 bits.

The signal flow inside the AD9850 is fascinating:

![AD9850 signal flow](/assets/amiq/ad9850_signal_flow.png)

It works as follows:

* each clock cycles, a phase accumulator programmable 32-bit word.
* the upper bits of the phase accumulator serves as address to a sine waveform table.
* a DAC converts the digital sine wave for analog.
* the analog signal is sent through a low-pass filter.
* the output of the low-pass filter goes through a comparator to generate a digital clock.

This thing has its own programmable signal generator in it! But why the roundtrip from
